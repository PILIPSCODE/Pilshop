...
import AddreesC from "./AddressInput";
...
const Storeauth = (props: { acc: any }) => {
  ...
  const [userReq, setuserReq] = useState({
    ...
    bio: `Welcome To ${props.acc?.name} Store`,
    link: `https://${props.acc?.name}.com`,
    Adress:""
  });
  ...
  return (
    <div className="h-screen flex justify-center items-center text-white font-poppins">
      <div className="w-10/12 h-5/6  flex  rounded-2xl gap-1  max-lg:flex-col max-lg:max-w-sm max-lg:h-screen max-sm:w-full   ">
        <div className="flex justify-center items-center h-full max-lg:h-auto max-sm:flex-grow relative  px-20 bg-log rounded-2xl ">
          <Image
            src={shopbag}
            className="absolute z-50 w-64 max-lg:w-24 bottom-3 right-3"
            alt="oke"
          />

          <Bird wings={false} change={change} showpasss={false} />
        </div>
        <div className="flex justify-center p-20 items-center  rounded-2xl max-lg:p-4   relative max-sm:flex-grow-0 flex-grow bg-gradient-to-b from-orange-500 to-orange-900  ">
          <form
            onSubmit={(e) => handleSubmit(e)}
            className=" flex flex-col gap-4 max-lg:w-full"
          >
            <Image
              alt="daun"
              src={Leaves}
            ...
            <div>
              <h1 className="relative z-50">Store Addrees</h1>
              <textarea
                required
                value={userReq.bio}
                onChange={(e) => {
           ...
           <AddreesC userReq={userReq} setuserReq={setuserReq}/>
            <button className="btn relative mt-2 max-lg:mx-3 ">
              Create Store
            </button>
          </form>
        </div>
